var request = require('request');

var LivecoinAdapter = {
    name: 'LiveCoin',
    provides: [
        ['golos', 'btc']
    ],
    get_pair: function(from,to,callback) {
        // console.log(from,to);

        var pair = [from.toUpperCase(),to.toUpperCase()].join('/'),
            ticker_url = 'https://api.livecoin.net/exchange/ticker/?currencyPair=' + pair;
        
        request(ticker_url, function(error,response,body) {
            if(error || response.statusCode != 200) {
                console.error('Invalid response code or server error:',error,response.statusCode);
                return callback(true,null);
            }
            var ticker_data = JSON.parse(body);
            if(!('last' in ticker_data)) {
                return callback(true,null);
            }
            return callback(false, ticker_data['last']);

        });
    }
}

module.exports = LivecoinAdapter;
